...
# txt dosyasındaki cümleleri okuyun ve spaCy ile işleyin
...
# Derlemin içerisinde en çok geçen 20 kelimeyi bulun
...
# En çok geçen 20 kelimenin her biri için en benzer 5 kelimeyi bulun
similar_words_dict = {}
...
# Her bir seçili cümle için en benzer 3 cümleyi bulun
similar_sentences_dict = {}
for i, selected_sentence in enumerate(selected_sentences):
    similar_sentences = []
    if selected_sentence:  # Check if selected_sentence is non-empty
        for j, sentence in enumerate(tokenized_sentences):
            if i != j and sentence and selected_sentence:  # Kendi cümlesi hariç ve karşılaştırılan cümle boş değilse
...
# Seçili cümleleri ve benzer cümleleri yazdırın
